...
import type { ProductHistoryItem, OverhaulHistoryItem, Alternative, RetailLink } from '../types';
import { HistoryIcon, SparklesIcon, AlertTriangleIcon, CheckCircleIcon, TrashIcon, LinkIcon, ShoppingBagIcon } from './common/Icon';
    ...
    <Card className="!p-0 overflow-hidden transition-shadow hover:shadow-xl">
        <div className="p-4 flex gap-4">
            <div className="flex-shrink-0 w-20 h-20">
                <ImageWithFallback src={item.imageUrl} alt={item.productName} className="w-full h-full object-cover rounded-md bg-slate-100" />
            </div>
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                        <p className="font-bold text-lg font-manrope truncate" title={item.productName}>{item.productName}</p>
                 ...
                 {item.allergens.length > 0 && (
                    <div className="mt-2 text-xs flex items-center p-1.5 bg-amber-50 border border-amber-200 rounded-md">
                        ...
                        <span className="font-semibold text-amber-800 truncate">Allergens: {item.allergens.join(', ')}</span>
                    </div>
                )}
            </div>
        </div>
         <details className="text-sm border-t border-slate-200">
            ...
            <div className="p-4 bg-slate-50 space-y-4">
                <div>
                 ...
                 {item.retailLinks && item.retailLinks.length > 0 && (
                    <div>
                        <h4 className="font-semibold flex items-center mb-2"><ShoppingBagIcon className="text-sky-500 mr-2 h-5 w-5" /> Where to Buy</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                             {item.retailLinks.map((link: RetailLink, i) => (
                                <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:shadow-md hover:border-sky-300 transition-all">
                                    <LinkIcon className="h-4 w-4 text-slate-400" />
                                    <span className="text-xs font-semibold text-slate-700">Shop at {link.retailer}</span>
                                </a>
                            ))}
                        </div>
                    </div>
