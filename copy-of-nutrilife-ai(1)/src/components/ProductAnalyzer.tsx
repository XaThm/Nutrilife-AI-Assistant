...
import type { AnalysisResult, ProductAnalysis, RetailLink } from '../types';
import { useHistory } from '../hooks/useHistory';
import Spinner from './common/Spinner';
import Card from './common/Card';
import { UploadIcon, AlertTriangleIcon, CheckCircleIcon, XCircleIcon, SparklesIcon, ZapIcon, BeakerIcon, InformationCircleIcon, XIcon, LinkIcon, ShoppingBagIcon } from './common/Icon';
import { motion, AnimatePresence } from 'framer-motion';
import ImageWithFallback from './common/ImageWithFallback';

const MAX_IMAGE_DIMENSION = 1024;

// This is a neat little trick to resize images on the client-side
// before uploading them. It saves bandwidth and makes the AI's job easier.
...
const ScoreBadge: React.FC<{ score: ProductAnalysis['overallScore'] }> = ({ score }) => {
    ...
    return (
        <div className={`text-center p-4 rounded-xl border-2 w-full ${scoreInfo[score].color}`}>
            <p className="text-6xl font-bold font-manrope">{score}</p>
            <p className="text-md font-semibold mt-1">{scoreInfo[score].text}</p>
        </div>
    );
};
...
const ProductAnalyzer: React.FC = () => {
    ...
    const { addProductToHistory, isHistoryEnabled } = useHistory();

    const runAnalysis = useCallback(async (query: string, file: File | null) => {
        if (!query.trim() && !file) {
        ...
        try {
            ...
            if (file) {
                const resized = await resizeAndEncodeImage(file);
    ...
    }, [addProductToHistory, isHistoryEnabled]);

    const handleAlternativeClick = useCallback((alternativeName: string) => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
        setProductInput(alternativeName);
        setImageFile(null);
        setImagePreview(null);
        runAnalysis(alternativeName, null);
    }, [runAnalysis]);
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            setProductInput(current => current || file.name.split('.')[0].replace(/_/g, ' '));
        }
    };
    
    const handleSubmit = useCallback((e: React.FormEvent) => {
        e.preventDefault();
        runAnalysis(productInput, imageFile);
    }, [runAnalysis, productInput, imageFile]);
            ...
            <AnimatePresence>
                ...
                {result && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { staggerChildren: 0.1 } }} className="mt-8 space-y-6">
                            ...
                            <div className="grid md:grid-cols-12 gap-8 items-start">
                                ...
                                <div className="md:col-span-8 lg:col-span-9">
                                    <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                                        ...
                                        <div className="flex-shrink-0 w-full sm:w-auto md:w-32">
                        ...
                        {result.retailLinks && result.retailLinks.length > 0 && (
                            <Card>
                                <h3 className="text-2xl font-bold font-manrope mb-4 flex items-center">
                                    <ShoppingBagIcon className="text-sky-500 mr-2 h-6 w-6" /> Where to Buy
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    {result.retailLinks.map((link: RetailLink, i) => (
                                        <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200 hover:shadow-md hover:border-sky-300 hover:bg-sky-50 transition-all">
                                            <LinkIcon className="h-5 w-5 text-slate-400" />
                                            <span className="font-semibold text-slate-700">Shop at {link.retailer}</span>
                                        </a>
                                    ))}
                                </div>
                            </Card>
                        ...
                        <Card>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {result.alternatives.map((alt, i) => (
                                    <div key={i} className="p-4 bg-slate-50 rounded-lg border border-slate-200 flex flex-col hover:shadow-md hover:border-sky-200 transition-all">
                                        <div className="flex-grow">
                                            <h4 className="font-semibold text-sky-800">{alt.productName}</h4>
                                            <p className="text-slate-600 text-sm mt-1">{alt.reason}</p>
                                        </div>
                                        <button
                                            onClick={() => handleAlternativeClick(alt.productName)}
                                            className="mt-4 w-full flex items-center justify-center gap-2 text-sm font-medium text-sky-700 bg-sky-100 hover:bg-sky-200 p-2 rounded-md transition-colors"
                                            aria-label={`Analyze ${alt.productName}`}
                                        >
