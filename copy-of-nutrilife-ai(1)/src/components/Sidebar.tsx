...
import { BeakerIcon, HomeIcon, HistoryIcon, XIcon, LogoutIcon, ClipboardListIcon } from './common/Icon';
...
// A more "human" way to write a component, using a function declaration
// instead of a const with an arrow function.
function NavLink({ icon, label, isActive, onClick, badgeCount }: NavLinkProps) {
  return (
    <a
      href="#"
      onClick={(e) => {
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, activeView, setActiveView, historyCount }) => {
  ...
  // Changed the order to be more logical: Dashboard, Analyzer, Overhaul, History
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: <HomeIcon className="h-5 w-5" /> },
    { id: 'analyzer', label: 'Product Analyzer', icon: <BeakerIcon className="h-5 w-5" /> },
    { id: 'overhaul', label: 'Lifestyle Overhaul', icon: <ClipboardListIcon className="h-5 w-5" /> },
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white">
        <div className="flex items-center justify-between px-4 h-20 border-b border-slate-100">
             <div className="flex items-center space-x-3">
                 <div className="w-10 h-10 bg-sky-500 rounded-full flex items-center justify-center shadow-lg shadow-sky-500/30">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" >
      ...
      <div className="flex-1 p-4 flex flex-col justify-between">
        ...
        <div>
           <div className="p-3 mb-2 rounded-lg bg-slate-100 text-center">
           ...
           <a
              ...
              className="group flex items-center w-full px-3 py-2.5 text-sm font-medium rounded-lg text-slate-600 hover:bg-red-100 hover:text-red-700 transition-colors duration-200"
            >
              <LogoutIcon className="h-5 w-5" />
              <span className="ml-3 flex-1">Logout</span>
            </a>
        </div>
      </div>
